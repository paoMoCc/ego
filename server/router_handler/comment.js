...
exports.eval = (req, res) => {
    ...
    // 根据orderid获取proid
    db.query(`select proId from orders,cart where orderId=${orderId} and orders.carId=cart.carId`, (err_1, result_1) => {
        if (err_1) return res.cc(err_1)
        if (result_1.length !== 1) return res.cc('获取proid失败！', 500)
        let proId = result_1[0].proId
        // 插入评论
        db.query(`insert into comment set ?`, { proId: proId, userId: req.auth.userId, orderId: orderId, content: content }, (err_2, result_2) => {
            if (err_2) return res.cc(err_2)
            if (result_2.affectedRows !== 1) return res.cc('评价失败！', 500)
            // 修改订单状态为已评价（status=50）
            db.query(`update orders set status=50,updateTime=? where userId=${req.auth.userId} and orderId=${orderId}`, getCurrentTime, (err_3, result_3) => {
